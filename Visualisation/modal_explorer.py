...
class ModalSliderExplorer(QMainWindow):
    def __init__(self, initial: Optional[ModalState] = None):
        ...
        if self.state.mesh_path and self.state.control_nodes_path:
            self.load_case(
    ...
    def _build_ui(self):
        ...
        self.amp_spin.setValue(self.state.amplitude)
        settings_form.addRow("Modal amplitude", self.amp_spin)
        ...
        build_btn = QPushButton("Build / rebuild modal basis")
        ...
        try:
            ...
            if control_normals_path:
                ...
                if self.control_normals.shape != self.control_nodes.shape:
                    raise ValueError(
                        f"control_normals shape {self.control_normals.shape} does not match "
                        f"control_nodes shape {self.control_nodes.shape}"
                    )
            else:
            ...
            max_modes = max(1, self.control_nodes.shape[0] - 1)
    ...
    def on_build_basis(self):
        if self.control_nodes is None:
            return
        try:
            self.evals, self.phi = build_knn_laplacian_basis(
                self.control_nodes,
                k_modes=int(self.k_spin.value()),
                knn=int(self.knn_spin.value()),
            )
...
def parse_args() -> ModalState:
    return ModalState(
        mesh_path=r"C:\Users\joell\OneDrive - Swansea University\Desktop\PhD Documents\01-Codes\Aeropt2\examples\sphere_opt_lap\surfaces\output.vtk",
        control_nodes_path=r"C:\Users\joell\OneDrive - Swansea University\Desktop\PhD Documents\01-Codes\Aeropt2\examples\sphere_opt_lap\Control Nodes\control_nodes.npy",
        control_normals_path=r"C:\Users\joell\OneDrive - Swansea University\Desktop\PhD Documents\01-Codes\Aeropt2\examples\sphere_opt_lap\Control Nodes\control_normals.npy",
        k_modes=5,
        knn=3,
        amplitude=1.0,
        deform_scale=25.0,
